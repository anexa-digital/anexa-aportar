...
public class OdooLiveChatServiceImpl implements OdooLiveChatService {
	...
	public void sendMessageToOdoo(int channel, String db, String user, String pwd, WaBoxAppRequestDto request) {
		...
		try {
			for (int i = 0; true; i++) {
				...
				val session = sessionService.getMailSession(channel, db, user, pwd, channelPhoneNumber,
						contactPhoneNumber);
				...
				try {
					...
					log("sessionService.execute(db, uid, pwd, MODEL_MAIL_CHANNEL, METHOD_MESSAGE_POST, args, odooMessage), session:"
							+ session.getResId(), request);
					...
					if (i < odooProperties.getMaxAttemptCount()) {
						sessionService.deleteSession(channel, db, session.getChannelPhoneNumber(),
								session.getContactPhoneNumber());
					} else {
						throw e;
					}
				}
			...
			// TODO SI SE REVIENTA SE DEBE AVISAR ES A TI DE APORTAR por medio un correo
			if (e instanceof SessionConnectionRefusedException) {
				System.out.println("No ha sido posible contactar a un operador disponible para atender su solicitud.");
			} else if (e instanceof SessionDeletedException) {
				System.out.println("No se pudo restablecer la conversación con el o con otro operador disponible.");
			} else {
				System.out
						.println("En estos momentos estamos teniendo un problema técnicos con la plataforma WhatsApp.");
			}
	...
	protected boolean discard(WaBoxAppRequestDto request) {
		...
		if (!"i".equalsIgnoreCase(request.getMessage().getDirection())) {
			return true;
		}
	...
	private String shortCodify(String body) {
		String result;
		if (body != null) {
